...
static int L3_read_side_info(bs_t *bs, L3_gr_info_t *gr, const uint8_t *hdr)
{
    ...
    do
    {
        ...
        gr->preflag = HDR_TEST_MPEG1(hdr) ? (uint8_t)get_bits(bs, 1) : (gr->scalefac_compress >= 500);
        ...
        } else
        {
            ...
            } else
            {
                ...
                for (k = 0; k < cnt; k++)
                {
                    ...
                    ist_pos[k] = (s == max_scf ? -1 : (uint8_t)s);
                    scf[k] = (uint8_t)s;
                }
            }
        }
...
static float L3_ldexp_q2(float y, int exp_q2)
{
    ...
    do
    {
        ...
        y *= g_expfrac[e & 3] * (float)(1 << 30 >> (e >> 2));
    } while ((exp_q2 -= e) > 0);
    return y;
}
...
static void L3_intensity_stereo(float *left, uint8_t *ist_pos, const L3_gr_info_t *gr, const uint8_t *hdr)
{
    ...
    for (i = 0; i < max_blocks; i++)
    {
        ...
        ist_pos[itop] = max_band[i] >= prev ? (uint8_t)default_pos : ist_pos[prev];
    }
...
#define V2(k) { VLOAD(k) b = VADD(b, VADD(VMUL(vz, w1), VMUL(vy, w0))); a = VADD(a, VSUB(VMUL(vy, w1), VMUL(vz, w0))); }
...

        {
            ...
            dstr[(15 - i)*nch] = (mp3d_sample_t)_mm_extract_epi16(pcm8, 1);
            dstr[(17 + i)*nch] = (mp3d_sample_t)_mm_extract_epi16(pcm8, 5);
            dstl[(15 - i)*nch] = (mp3d_sample_t)_mm_extract_epi16(pcm8, 0);
            dstl[(17 + i)*nch] = (mp3d_sample_t)_mm_extract_epi16(pcm8, 4);
            dstr[(47 - i)*nch] = (mp3d_sample_t)_mm_extract_epi16(pcm8, 3);
            dstr[(49 + i)*nch] = (mp3d_sample_t)_mm_extract_epi16(pcm8, 7);
            dstl[(47 - i)*nch] = (mp3d_sample_t)_mm_extract_epi16(pcm8, 2);
            dstl[(49 + i)*nch] = (mp3d_sample_t)_mm_extract_epi16(pcm8, 6);
#else /* HAVE_SSE */
            int16x4_t pcma, pcmb;
            a = VADD(a, VSET(0.5f));
            b = VADD(b, VSET(0.5f));
